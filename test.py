...
def cubic_lagrange_interpolation(x, x_nodes, y_nodes):
    ...
    if x == x_nodes[0]:  # Если x равно первому узлу
        x0, x1, x2, x3 = x_nodes[0], x_nodes[1], x_nodes[2], x_nodes[3]
    elif x == x_nodes[-1]:  # Если x равно последнему узлу
        x0, x1, x2, x3 = x_nodes[-4], x_nodes[-3], x_nodes[-2], x_nodes[-1]
...
table_data = {'x': x_values, 'f(x)': f(x_values).round(4), 'Cubic Lagrange Interpolation': cubic_lagrange_values.flatten()}
